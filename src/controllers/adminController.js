...
const updateUserToAdmin = async(req, res) => {
    ...
    if(!is_admin === true) {
        errorMessage.error = 'Sorry you are unauthorized to make user and admin';
        return res.status(status.bad).send(errorMessage);
    }
    if(isAdmin === ''){
    ...
    try{
        const { rows } = await dbQuery.query(findQuery, [id]);
        const dbResponse = rows[0];
        if(!dbResponse){
        ...
        errorMessage.error = 'Operation was not successfull';
